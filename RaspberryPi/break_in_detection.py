...
def run_detection_script():
    global process
    command = "python"
    #script = "yolov5\detect.py" # in Windows
...
def analyze_results():
    ...
    while running:
        try:
            ...
            # Reset notification status after 10 mins
            ...
            if len(df) > last_checked:
                ...
                for label in enumerate(new_labels):
                    if label == 'fall detected' or 'sitting' or 'walking':
                        frame_count += 1
                        if frame_count == 3:
...
def send_notification(status):
    ...
    information = 'Human Detected around the Door'
    if status is True:
...
if __name__ == "__main__":
    ...
    MYTRANSPORT = 'tcp'
    TOPIC_PUB = 'pub/break_in_detection' # as a subscriber and publisher
    TOPIC_SUB = 'sub/break_in_detection'
